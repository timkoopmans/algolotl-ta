...
impl<T> Window<T> {
    ...
    ///
    /// Returns the oldest pushed value.
    ///
    /// # Panics
    ///
    ...
    pub fn push(&mut self, value: T) -> T {
        ...
        let refer = &mut self.buf[self.index as usize];
...
impl<T> std::ops::Index<PeriodType> for Window<T> {
    ...
    fn index(&self, index: PeriodType) -> &Self::Output {
        let buf_index = self
            .slice_index(index)
            .unwrap_or_else(|| panic!("Window index {index} is out of range"))
            as usize;
...
mod tests {
    use super::*;
    use crate::candles::RandomCandles;
